...
const createListElems = tasksList => {
  listElem.innerHTML = "";
  const tasksElems = tasksList.sort((el1, el2) => el1.done - el2.done || new Date(el2.timeOfChange) - new Date(el1.timeOfChange)).map(createListItem);
  listElem.append(...tasksElems);
};

export const renderTasks = () => {
  getTasksList().then(tasksList => createListElems(tasksList));
};